...
public class HeightmapFromTexture
{
	...
	public static void ApplyHeightmap() {
		...
		if (heightmap == null) {
			EditorUtility.DisplayDialog("No texture selected", "Please select a texture and terraindata.", "Cancel");
			return;
		}
		if (terrain == null) {
			EditorUtility.DisplayDialog("No terrain selected", "Please select a texture and terraindata.", "Cancel");
			return;
		}
		Undo.RegisterCompleteObjectUndo(terrain, "Heightmap From Texture");
		...
		for (int y = 0; y < w2; y++) {
			for (int x = 0; x < w2; x++) {
				heightmapData[y, x] = map[y * w2 + x].grayscale * 2;
			}
		}
