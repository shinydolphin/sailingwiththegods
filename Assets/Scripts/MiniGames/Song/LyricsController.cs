...
public class LyricsController : MonoBehaviour
{
	...
	void Awake()
    {
		...
		StreamReader SR = new StreamReader(Application.persistentDataPath + "\\wisdomy.txt");
		// Finding which lyrics to use
		if (SR!=null) 
		{
			while (!SR.EndOfStream) 
			{
				wisdomyLyricsList.Add(SR.ReadLine());
			}
			SR.Close();
		}
		else
		{
			Debug.Log("file not found defalting songs");
			//Wisdom
		...
		SR = new StreamReader(Application.persistentDataPath + "\\military.txt");
		if (SR != null) {
			while (!SR.EndOfStream) {
				militaryLyricsList.Add(SR.ReadLine());
			}
			SR.Close();
		}
		else 
	   ...
	   " (Aristophanes Knights, second semi-chorus)");
		...
		SR = new StreamReader(Application.persistentDataPath + "\\mournful.txt");
		if (SR != null) {
			while (!SR.EndOfStream) {
				mournfulLyricsList.Add(SR.ReadLine());
			}
			SR.Close();
		}
		else 
		...
		SR = new StreamReader(Application.persistentDataPath + "\\party.txt");
		if (SR != null) {
			while (!SR.EndOfStream) {
				partyLyricsList.Add(SR.ReadLine());
			}
			SR.Close();
		}
		else 
